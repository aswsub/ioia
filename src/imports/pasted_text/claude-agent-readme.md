...
Ioia name https://www.thedictionaryofobscuresorrows.com/word/ioia

Jobs for Us 5/1 Tonight
Sid - Prompts and CLAUDE/AGENT.md files
Aswath - Setup github and work on prompts/Git setup/tooling/repo
Rahul - Figma make and UI wireframe
...
Uploads a resume + a writing sample → we extract a structured profile (interests, experience, tone).
Picks an opportunity type (research, internship) and a target university.
Gets a list of relevant professors with recent papers and concept matches.
Reviews an AI-drafted cold email per professor that reflects their voice and references real work.
Sends it via Gmail (demo: mailto: link), and tracks state on a dashboard.
...
Etiquette block — static constant in lib/prompts/etiquette.ts. Norms for emailing professors: subject line conventions, paper references, length (under 150 words for research outreach), the ask, signature. Don't compute this. It's a constant.
Tone block — interpolated from UserProfile.tone. Tells Claude to mirror specific style traits.
Context block — the professor's recent papers and the user's relevant experience.
...
Check the author's homepage URL if present.
Try the university's firstname.lastname@domain pattern.
If both fail, surface email: null in the UI and tell the user to find it.
...
Errors return { error: string } with appropriate status. No stack traces to the client.
...
A "Connect Gmail" button on the dashboard that opens a modal, spinners for ~1.2s, flips to "Connected ✓" with a Google logo and a fake account email. State persists in localStorage.
The "Send" button on the email composer generates a mailto: link with subject and body pre-filled and opens it.
After "send," optimistically add the email to the mocked dashboard state.
...
Onboarding — paste resume PDF, paste writing sample → see extracted profile JSON render in a card. (~15s)
Search — pick a university, see professor cards with match scores and recent papers. (~20s)
Compose — click a professor, watch email stream in, edit one line, click send → mailto opens. (~25s)
Dashboard — show sent count, one mocked reply, follow-up suggestion. (~15s)
Reply flow (mocked) — flash a "reply received → calendar slot → prep mode with behavioral + DSA tips" screen. (~15s)
...
Read the relevant schema in lib/schemas/ before changing any data shape. Update the schema first, then the code that uses it.
Don't add new dependencies without a clear reason. Stack is fixed above.
Don't add a database, auth, or a Python service. Out of scope.
Don't refactor for "production readiness." This is a hackathon. Working > clean.
When editing prompts in lib/prompts/, preserve the existing structure (etiquette block, tone block, context block for the email writer). Hard rules in the email writer prompt are intentional — don't soften them.
Mocked dashboard data is intentional. Don't wire it to a real backend.
The mailto link is intentional. Don't replace with real send.
Run npm run dev to verify changes. Run npm run build before claiming a feature is done — type errors block the demo.
If you find yourself writing more than ~150 lines for a single feature, stop and ask. Probably overbuilding.
